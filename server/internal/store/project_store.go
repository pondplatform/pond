...
func (s *ProjectStore) GetByID(ctx context.Context, id uuid.UUID) (*domain.Project, error) {
	...
	err := s.db.QueryRowContext(ctx,
		"SELECT id, name, root_environment_id, created_at FROM projects WHERE id = $1", id,
	).Scan(&p.ID, &p.Name, &rootEnvID, &p.CreatedAt)
...
func (s *ProjectStore) GetByName(ctx context.Context, name string) (*domain.Project, error) {
	var p domain.Project
	var rootEnvID sql.NullString
	err := s.db.QueryRowContext(ctx,
		"SELECT id, name, root_environment_id, created_at FROM projects WHERE name = $1",
		name,
	).Scan(&p.ID, &p.Name, &rootEnvID, &p.CreatedAt)
	if err == sql.ErrNoRows {
...
func (s *ProjectStore) List(ctx context.Context) ([]domain.Project, error) {
	rows, err := s.db.QueryContext(ctx,
		"SELECT id, name, root_environment_id, created_at FROM projects",
	)
	if err != nil {
	...
	for rows.Next() {
		var p domain.Project
		var rootEnvID sql.NullString
		if err := rows.Scan(&p.ID, &p.Name, &rootEnvID, &p.CreatedAt); err != nil {
...
func (s *ProjectStore) Create(ctx context.Context, project *domain.Project) error {
	_, err := s.db.ExecContext(ctx,
		"INSERT INTO projects (id, name, root_environment_id, created_at) VALUES ($1, $2, $3, $4)",
		project.ID, project.Name, project.RootEnvironmentID, project.CreatedAt,
	)
	if err != nil {
